...
def test_ecapa_tdnn_c512_forward_shapes() -> None:
    ...
    frame_features, emb = model(torch.randn(2, 100, 80))
    assert frame_features.shape == (2, 1536, 100)
...
def test_ecapa_tdnn_c1024_forward_shapes() -> None:
    ...
    frame_features, emb = model(torch.randn(2, 100, 80))
    assert frame_features.shape == (2, 3072, 100)
    assert emb.shape == (2, 192)
...
def test_speaker_encoder_loads_wrapped_checkpoint(tmp_path) -> None:
    ...
    emb = model(torch.randn(2, 16000))
    assert emb.shape == (2, 192)
    assert model.backend.mfa.conv.conv.weight.shape[0] == 3072
...
def test_speaker_encoder_auto_detects_variant(tmp_path) -> None:
    ...
    emb = model(torch.randn(2, 16000))
    assert emb.shape == (2, 192)
    assert model.backend.blocks[0].conv.conv.weight.shape[0] == 1024


def test_speaker_encoder_rejects_non_speechbrain_checkpoint(tmp_path) -> None:
    checkpoint_path = tmp_path / "speaker.pt"
    torch.save({"frontend.conv.weight": torch.randn(8, 1, 3)}, checkpoint_path)
    with pytest.raises(ValueError, match="SpeechBrain ECAPA-TDNN"):
